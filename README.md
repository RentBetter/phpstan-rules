...
| `NoWallClockRule` | `ptgs.noWallClock` | The current time read off the wall — `new DateTimeImmutable()` / `new DateTime()` / `new DatePoint()` with no argument or one PHP resolves against now (`'today'`, `'+1 day'`, `'10:00'`), including one built by a constant, concatenation, interpolation or `sprintf()` (`"-{$days} days"`); `time()`; and `date()`, `gmdate()`, `idate()`, `localtime()`, `getdate()` without a timestamp — instead of an injected `Psr\Clock\ClockInterface` or `Symfony\Component\Clock\Clock::get()->now()`. Explicit values (`new DateTimeImmutable('2025-01-15')`) and `microtime()` stopwatches are fine |
